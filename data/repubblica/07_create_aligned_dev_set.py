...
if __name__ == "__main__":
    ...
    alignedFolder = "./alignedProva"
    replicatedFolder = "./replicatedProva"

    dev_dir = Path(f"{alignedFolder}/dev")
    dev_dir.mkdir(exist_ok=True)
    for fpath in Path(f"{alignedFolder}").glob("*"):
        if fpath.is_file():
            shutil.copy(str(fpath), str(Path(dev_dir, fpath.name)))

    # Load in the ids from the replicated fpath
    repl_train_ids = load_json(f"{replicatedFolder}/dev/train.ids.json")
    repl_dev_ids = load_json(f"{replicatedFolder}/dev/dev.ids.json")
